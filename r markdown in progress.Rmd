...
Define exactly what we want to know. Specifically, we want two columns - one column with the net change in the numbers of students enrolled between the two datasets, another with that change expressed as a percentage of the original number of students enrolled.



After we have a model, Temperance suggests we should:

We should recognize the potential limitations of our model. For eg., our dataset might be lacking in understanding the current number of students who are considering dropping a class.
...
```{r}
new <- left_join(sep_old, sep_new, by = "title", suffix = c("_old", "_new")) %>%
...
full_join(T5_increase, T5_decrease, by = c("title", "u_grad_old", "u_grad_new", "change")) %>%
  ggplot(aes(x=reorder(title, change), y=change, fill = title)) +
  geom_col() +
  labs(title="Biggest Changes in Course Enrolment", y="Change in Number of Students", x="Course Title")
```
